...
function Menu()
 ...
 -----------------------------COMBO-----------------------------------
     ...
     Configzedex.comboset:addParam("zedezq", "Use Q", SCRIPT_PARAM_ONOFF, true)
		 ...
		 Configzedex.comboset:addParam("laneclear", "LaneClear", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("G"))
	Configzedex.comboset:addParam("lasthit", "LastHit", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("X"))
	SxOrb:RegisterHotKey("Fight", Configzedex, "combozedex") 
		 SxOrb:RegisterHotKey("laneclear", Configzedex.comboset, "laneclear") 
	   SxOrb:RegisterHotKey("lasthit", Configzedex.comboset, "lasthit") 
...
ts:update()
	...
	if ValidTarget(target) then
		...
		if WREADY and WRANGE and Configzedex.comboset.zedexw and Configzedex.combozedex then
			CastSpell(_W, target.x, target.z)
		end

		if QREADY and QRANGE and Configzedex.combozedex then
 ...
 CastSpell(_W)
		...
		if RREADY and RRANGE and Configzedex.comboset.zedexr and Configzedex.combozedex then
			CastSpell(_R, target)
		end
	end
end
...
ts:update()
	...
	if ValidTarget(target) then
if QREADY and QRANGE and Configzedex.harass.harass then
			CastSpell(_Q, target.x, target.z)
		    end
		  end
		end
